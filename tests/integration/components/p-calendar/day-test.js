...
module('Integration | Component | p calendar/day', function(hooks) {
  ...
  test('it highlights days with possible issues', async function(assert) {
    ...
    setData();
    await render(
      hbs`{{p-calendar/day dayNameFull=dayNameFull mDay=mDay monthName=monthName shows=shows isEditing=true}}`,
    );

    setData([new ShowData(1, '2p', { 1: 'Some Show' }), new ShowData(2, '8p', { 1: 'Some Other Show' })], 'Sunday');
    ...
    setData([new ShowData(1, '2p', { 1: 'Some Show' }), new ShowData(2, '2p', { 1: 'Some Other Show' })]);
